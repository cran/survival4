...
double  *sx,
    ...
    */
    ...
    wvec  = (double *)S_alloc(ntime*ngrp+SAFETY, sizeof(double));
    for (i=0; i<edim; i++) {
	ecut[i] = secut;
	...
	*/
	for (j=0; j<ntime && timeleft >0; j++) {
		...
		etime -= et2;
/*
printf("time=%5.1f, rate1=%6e, rate2=%6e, wt=%3.1f, ind=%d\n", et2, expect[indx], expect[indx2], wt,index);
*/
		}
	    if (times[j]==0) {
		wvec[index]=1;
